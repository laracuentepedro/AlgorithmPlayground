...
export function LinkedListValuesPlayground() {
  ...
  const createLinkedList = (values: string[]): ListNode | null => {
  ...
  const linkedListToArray = (head: ListNode | null): string[] => {
    const result: string[] = [];
    let current = head;
    while (current) {
  ...
  const createVisualization = (values: string[], currentIndex: number = -1): string => {
    if (values.length === 0) {
  ...
  const linkedListValuesIterative = (values: string[]) => {
    const steps: AlgorithmStep[] = [];
    const result: string[] = [];
    
    steps.push({
      step: 1,
      description: "Initialize empty result array and set current pointer to head",
      details: `Starting traversal of linked list with ${values.length} nodes`,
      result: [],
      action: 'initialize',
      currentNode: values.length > 0 ? values[0] : null,
      linkedListVisualization: createVisualization(values, values.length > 0 ? 0 : -1),
      approach: 'iterative'
    });
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      steps.push({
        ...
        linkedListVisualization: createVisualization(values, i),
        approach: 'iterative'
      });
      ...
      steps.push({
        step: steps.length + 1,
        description: `Added "${currentVal}" to result array`,
        details: `Result array now contains: [${result.join(', ')}]`,
        result: [...result],
        action: 'add_value',
        currentNode: currentVal,
        linkedListVisualization: createVisualization(values, i),
      ...
      if (i < values.length - 1) {
        steps.push({
          ...
          linkedListVisualization: createVisualization(values, i + 1),
          approach: 'iterative'
        });
      }
    }
    ...
    steps.push({
      step: steps.length + 1,
      description: "Reached end of list (null), traversal complete",
      details: `Final result: [${result.join(', ')}]`,
      result: [...result],
      action: 'complete',
      currentNode: null,
      linkedListVisualization: createVisualization(values, -1),
      approach: 'iterative'
    });
  ...
  const linkedListValuesRecursive = (values: string[]) => {
    const steps: AlgorithmStep[] = [];
    const result: string[] = [];
    
    steps.push({
      step: 1,
      description: "Initialize empty result array for recursive approach",
      details: `Starting recursive traversal of linked list with ${values.length} nodes`,
      result: [],
      action: 'initialize',
      currentNode: values.length > 0 ? values[0] : null,
      linkedListVisualization: createVisualization(values, values.length > 0 ? 0 : -1),
    ...
    const traverse = (index: number, depth: number) => {
      if (index >= values.length) {
        steps.push({
          step: steps.length + 1,
          description: `Base case reached: node is null`,
          details: `Recursion depth ${depth}: Return from recursive call`,
          result: [...result],
          action: 'base_case',
          currentNode: null,
          linkedListVisualization: createVisualization(values, -1),
      ...
      steps.push({
        step: steps.length + 1,
        description: `Added "${currentVal}" to result array`,
        details: `Recursion depth ${depth}: Result array now contains: [${result.join(', ')}]`,
        result: [...result],
        action: 'add_value',
        currentNode: currentVal,
        linkedListVisualization: createVisualization(values, index),
    ...
    if (values.length > 0) {
      ...
      steps.push({
        ...
        details: `Empty linked list, returning empty array`,
        result: [],
        action: 'base_case',
        currentNode: null,
        linkedListVisualization: createVisualization(values, -1),
    ...
    steps.push({
      step: steps.length + 1,
      description: "Recursive traversal complete",
      details: `All recursive calls finished. Final result: [${result.join(', ')}]`,
      result: [...result],
      action: 'complete',
      currentNode: null,
      linkedListVisualization: createVisualization(values, -1),
  ...
  const runAlgorithm = () => {
    ...
    const { result: extractedValues, steps: algorithmSteps } = selectedApproach === 'iterative' 
      ? linkedListValuesIterative(values)
      : linkedListValuesRecursive(values);
  ...
  const runTestCase = (testValues: string[]) => {
    setInputValues(testValues.join(','));
    
    setTimeout(() => {
      const startTime = performance.now();
      const { result: extractedValues, steps: algorithmSteps } = selectedApproach === 'iterative' 
        ? linkedListValuesIterative(testValues)
        : linkedListValuesRecursive(testValues);
      const endTime = performance.now();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-3">
              ...
              <span className="hidden sm:inline text-sm text-slate-500 bg-slate-100 px-2 py-1 rounded-full">Linked List Values</span>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8 animate-fade-in">
          <div className="flex flex-col sm:flex-row sm:items-start space-y-3 sm:space-y-0 sm:space-x-4">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-cyan-100 rounded-xl flex items-center justify-center flex-shrink-0">
              <i className="fas fa-link text-cyan-600 text-base sm:text-lg"></i>
            </div>
            <div className="flex-1 min-w-0">
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-2 sm:mb-3">Linked List Values</h2>
              <p className="text-slate-700 text-base sm:text-lg leading-relaxed mb-3 sm:mb-4">
                Write a function that takes the head of a linked list and returns an array containing all values of the nodes. 
                This fundamental operation introduces linked list traversal patterns.
              </p>
                ...
                <div className="p-4 rounded-lg border-l-4 border-emerald-400 bg-emerald-50 animate-pulse-success flex items-center space-x-3">
                  ...
                  <div>
                    <div className="font-semibold text-emerald-800">
                      Extracted Values: [{result.join(', ')}]
                    </div>
                    <div className="text-sm opacity-75 text-emerald-700">
                      Successfully traversed linked list using {selectedApproach} approach
                    </div>
                  </div>
                </div>
          ...
          {showVisualization && steps.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              <div className="mb-6">
                ...
                <div 
                  className="p-4 bg-slate-50 rounded-lg min-h-[4rem] flex items-center"
              ...
              {/* Current Result */}
              <div>
                <h4 className="text-sm font-medium text-slate-700 mb-3">Current Result Array</h4>
                <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                  <span className="font-mono text-lg">
                    [{step.result.join(', ')}]
                  </span>
                  <span className="ml-3 text-sm text-emerald-600">
                    {step.result.length} value{step.result.length !== 1 ? 's' : ''} extracted
                  </span>
                </div>
              </div>
            </div>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="bg-slate-900 rounded-lg p-4 sm:p-6 overflow-x-auto mb-4">
                      <pre className="text-sm text-slate-300 font-mono">
                        <code>
                          <span className="text-blue-400">const</span> <span className="text-yellow-300">linkedListValues</span> = <span className="text-slate-400">(</span><span className="text-orange-300">head</span><span className="text-slate-400">) =&gt; {`{`}</span>{'\n'}
                          ...
                          {'    '}<span className="text-yellow-300">values</span><span className="text-slate-400">.</span><span className="text-yellow-300">push</span><span className="text-slate-400">(</span><span className="text-yellow-300">current</span><span className="text-slate-400">.</span><span className="text-yellow-300">val</span><span className="text-slate-400">);</span>{'\n'}
                    ...
                    <div className="bg-slate-900 rounded-lg p-4 sm:p-6 overflow-x-auto mb-4">
                      <pre className="text-sm text-slate-300 font-mono">
                        <code>
                          <span className="text-blue-400">const</span> <span className="text-yellow-300">linkedListValues</span> = <span className="text-slate-400">(</span><span className="text-orange-300">head</span><span className="text-slate-400">) =&gt; {`{`}</span>{'\n'}
                          {'  '}<span className="text-blue-400">const</span> <span className="text-yellow-300">values</span> = <span className="text-slate-400">[];</span>{'\n'}
                          {'  '}<span className="text-yellow-300">_linkedListValues</span><span className="text-slate-400">(</span><span className="text-orange-300">head</span><span className="text-slate-400">,</span> <span className="text-yellow-300">values</span><span className="text-slate-400">);</span>{'\n'}
                          {'  '}<span className="text-purple-400">return</span> <span className="text-yellow-300">values</span><span className="text-slate-400">;</span>{'\n'}
                          <span className="text-slate-400">{`};`}</span>{'\n'}
                          {'\n'}
                          <span className="text-blue-400">const</span> <span className="text-yellow-300">_linkedListValues</span> = <span className="text-slate-400">(</span><span className="text-orange-300">head</span><span className="text-slate-400">,</span> <span className="text-orange-300">values</span><span className="text-slate-400">) =&gt; {`{`}</span>{'\n'}
                          {'  '}<span className="text-purple-400">if</span> <span className="text-slate-400">(</span><span className="text-orange-300">head</span> === <span className="text-blue-400">null</span><span className="text-slate-400">)</span> <span className="text-purple-400">return</span><span className="text-slate-400">;</span>{'\n'}
                          {'  '}<span className="text-orange-300">values</span><span className="text-slate-400">.</span><span className="text-yellow-300">push</span><span className="text-slate-400">(</span><span className="text-orange-300">head</span><span className="text-slate-400">.</span><span className="text-yellow-300">val</span><span className="text-slate-400">);</span>{'\n'}
                          {'  '}<span className="text-yellow-300">_linkedListValues</span><span className="text-slate-400">(</span><span className="text-orange-300">head</span><span className="text-slate-400">.</span><span className="text-yellow-300">next</span><span className="text-slate-400">,</span> <span className="text-orange-300">values</span><span className="text-slate-400">);</span>{'\n'}
                          <span className="text-slate-400">{`};`}</span>
                        </code>
                      </pre>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                      ...
                      <div className="text-sm text-emerald-700 space-y-1">
                        ...
                        <div><strong>Space:</strong> O(n) - Result array storage</div>
                        <div className="text-xs text-emerald-600 mt-2">Uses explicit loop with current pointer</div>
                      </div>
                    </div>
                    <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                      ...
                      <div className="text-sm text-purple-700 space-y-1">
                        <div><strong>Time:</strong> O(n) - Visit each node once</div>
                        <div><strong>Space:</strong> O(n) - Call stack + result array</div>
                        <div className="text-xs text-purple-600 mt-2">Uses function call stack for traversal</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    ...
                    <ul className="text-sm text-yellow-700 space-y-1 list-disc list-inside">
                      <li>Both approaches visit each node exactly once for optimal time complexity</li>
                      <li>Iterative uses while loop with explicit current pointer management</li>
                      <li>Recursive naturally follows the linked list structure with function calls</li>
                      <li>Essential pattern for all linked list traversal problems</li>
                    </ul>
                  </div>
                </div>
              )}
