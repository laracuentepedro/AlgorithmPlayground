...
export function PairSumPlayground() {
  ...
  const pairSumWithSteps = (numbers: number[], targetSum: number) => {
    ...
    steps.push({
      ...
      details: `Looking for two numbers that sum to ${targetSum}`,
      previousNums: {},
      action: 'initialize',
      currentIndex: -1,
      currentNum: 0,
    ...
    for (let i = 0; i < numbers.length; i++) {
      ...
      steps.push({
        ...
        details: `Need to find complement: ${targetSum} - ${num} = ${complement}`,
      ...
      if (complement in previousNums) {
        ...
        steps.push({
          ...
          details: `numbers[${complementIndex}] + numbers[${i}] = ${numbers[complementIndex]} + ${num} = ${targetSum}`,
          previousNums: { ...previousNums },
          action: 'found_pair',
  ...
  const runAlgorithm = () => {
    ...
    if (numbers.length < 2 || isNaN(target)) {
      alert('Please enter a valid array (comma-separated numbers) and target sum');
      return;
    }

    const startTime = performance.now();
    const { result: indices, steps: algorithmSteps } = pairSumWithSteps(numbers, target);
  ...
  const runTestCase = (testArray: number[], testTarget: number) => {
    ...
    setTargetSum(testTarget.toString());
    
    setTimeout(() => {
      const startTime = performance.now();
      const { result: indices, steps: algorithmSteps } = pairSumWithSteps(testArray, testTarget);
      const endTime = performance.now();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-3">
              ...
              <span className="hidden sm:inline text-sm text-slate-500 bg-slate-100 px-2 py-1 rounded-full">Pair Sum</span>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8 animate-fade-in">
          <div className="flex flex-col sm:flex-row sm:items-start space-y-3 sm:space-y-0 sm:space-x-4">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-violet-100 rounded-xl flex items-center justify-center flex-shrink-0">
              <i className="fas fa-plus text-violet-600 text-base sm:text-lg"></i>
            </div>
            <div className="flex-1 min-w-0">
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-2 sm:mb-3">Pair Sum (Two Sum)</h2>
              <p className="text-slate-700 text-base sm:text-lg leading-relaxed mb-3 sm:mb-4">
                Write a function that takes an array and a target sum. Return the indices of two numbers that add up to the target. 
        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-6">
              ...
              <div>
                <Label htmlFor="targetSum" className="block text-sm font-medium text-slate-700 mb-2">
                  Target Sum
                </Label>
                <Input
                  id="targetSum"
                ...
                <div className="p-4 rounded-lg border-l-4 border-emerald-400 bg-emerald-50 animate-pulse-success flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm opacity-75 text-emerald-700">
                      Elements: {parseArray(inputArray)[result[0]]} + {parseArray(inputArray)[result[1]]} = {targetSum}
                    </div>
                  </div>
                </div>
              ...
              {step.currentIndex >= 0 && (
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <h4 className="font-semibold text-blue-800 mb-2">Current Element</h4>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-slate-900 rounded-lg p-4 sm:p-6 overflow-x-auto">
                    <pre className="text-sm text-slate-300 font-mono">
                      <code>
                        <span className="text-blue-400">const</span> <span className="text-yellow-300">pairSum</span> = <span className="text-slate-400">(</span><span className="text-orange-300">numbers</span><span className="text-slate-400">,</span> <span className="text-orange-300">targetSum</span><span className="text-slate-400">) =&gt; {`{`}</span>{'\n'}
                        ...
                        {'    '}<span className="text-blue-400">const</span> <span className="text-yellow-300">complement</span> = <span className="text-orange-300">targetSum</span> - <span className="text-yellow-300">num</span><span className="text-slate-400">;</span>{'\n'}
                        {'    '}{'\n'}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                      ...
                      <ol className="text-sm text-emerald-700 space-y-1 list-decimal list-inside">
                        ...
                        <li>For each element, calculate complement</li>
                  ...
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    ...
                    <ul className="text-sm text-yellow-700 space-y-1 list-disc list-inside">
                      <li>Hash map enables O(1) complement lookup instead of O(n) nested loop</li>
                      <li>Store index as value to return indices, not the elements themselves</li>
                      <li>Check for complement before storing current number to avoid self-pairing</li>
                      <li>This approach improves from O(n²) brute force to O(n) optimal solution</li>
                    </ul>
                  </div>
                </div>
              )}
